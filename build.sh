...
TARGET_ARCH="arm64"
KERNEL_CONFIG="msm8937-perf_defconfig"
BASE_DTB_NAME="sm8150-v2"
ZIP_KERNEL_STR="mochi"
ZIP_DEVICE_NAME="TB-8504X"
ZIP_PREFIX_STR="mochi-TB-8504X"
...
UFDT_ARGS="create dtbo.img --page_size=4096 $DTB_DTBO_DIR/*.dtbo"
...
# 	echo -e "\nDone."
	...
	if [[ ${1} != "" && ${1} == "dtbs" ]]; then
		...
		if [[ -f $DTB_DTBO_DIR/${BASE_DTB_NAME}.dtb ]]; then
				echo -e "\nCopying dtb..."
				cp "$DTB_DTBO_DIR/${BASE_DTB_NAME}.dtb" "$OUT_BOOT_DIR/dtb.img"
		fi
		exit
	fi

	run "PATH=$PATH CROSS_COMPILE=$CROSS_COMPILE CROSS_COMPILE_ARM32=$CROSS_COMPILE_ARM32 make $(echo -e $MAKEOPTS) O=out ARCH=$TARGET_ARCH CC=\"${NEW_CC}\" -j$(($(nproc)+8)) || exit"
...
build_zip() {
	...
	(
		...
		if [[ -f $DTB_DTBO_DIR/${BASE_DTB_NAME}.dtb ]]; then
				echo -e "\nCopying dtb..."
				cp "$DTB_DTBO_DIR/${BASE_DTB_NAME}.dtb" "$ANYKERNEL_DIR/dtb.img"
		fi
