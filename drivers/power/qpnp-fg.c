...
#if defined(CONFIG_KERNEL_CUSTOM_P3592_DEBUG)//add by longcheer_liml_2016_12_13_for_open log
#define DEBUG
#endif
...
static void status_change_work(struct work_struct *work)
{
	...
	if (chip->status == POWER_SUPPLY_STATUS_FULL) {
		printk("zbl enter power supply full,capacity is %d\n",capacity);
		if (capacity >= 99 && chip->hold_soc_while_full
				&& chip->health == POWER_SUPPLY_HEALTH_GOOD) {
			if (fg_debug_mask & FG_STATUS)
				pr_info("holding soc at 100\n");
			chip->charge_full = true;
			printk("zbl chip->charge_full is true\n");
...
static void charge_full_work(struct work_struct *work)
{
	...
	if (chip->status != POWER_SUPPLY_STATUS_FULL) {
		if (fg_debug_mask & FG_STATUS)
			pr_info("zbl battery not full: %d\n", chip->status);
		disable = true;
	}
	...
	if (buffer[2] <= resume_soc_raw) {
		if (fg_debug_mask & FG_STATUS)
			pr_info("zbl bsoc = 0x%02x <= resume = 0x%02x\n",
...
out:
	...
	if (disable){
		...
		printk("zbl chip->charge_full set to false\n");
		}
}
