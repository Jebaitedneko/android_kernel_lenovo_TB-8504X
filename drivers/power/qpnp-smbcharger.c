...
#if defined(CONFIG_KERNEL_CUSTOM_P3590)  || defined(CONFIG_KERNEL_CUSTOM_P3592) //modify_liml_2016_12_15_for battery runin test
						...
						const char *client)
{
	...
	if (icl_ma > aicl_ma
		#ifdef CONFIG_TUSB422
		&& (!((chip->usb_supply_type == POWER_SUPPLY_TYPE_UNKNOWN) && chip->usb_pd_flag))
		#endif
		)
		smbchg_rerun_aicl(chip);
...
#if defined(CONFIG_KERNEL_CUSTOM_P3590)  || defined(CONFIG_KERNEL_CUSTOM_P3592)
/*2017-03-06 Bert add for usb prevent short*/
	if(usb_supply_type == POWER_SUPPLY_TYPE_USB_DCP)	//only DCP mode start usb prevent short
	{
		queue_delayed_work(chip->usb_conn_check_workqueue, &chip->usb_conn_check_work, 2*HZ);
	}
#endif
