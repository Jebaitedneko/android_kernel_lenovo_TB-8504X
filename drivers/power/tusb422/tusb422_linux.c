...
#endif
#if 0	//def CONFIG_DUAL_ROLE_USB_INTF
...
#endif
	struct pinctrl *tusb422_pinctrl;
...
void external_gpio_direction_output(int value)
{
	gpio_direction_output(tusb422_pd->usb_id, value);
}
#ifdef TUSB422_DEBUG
...
#define PINCTRL_STATE_ACTIVE "tusb_active"
#define PINCTRL_STATE_SUSPEND "tusb_suspend"
static int tusb422_pinctrl_init(struct tusb422_pwr_delivery *tusb422_pd)
{
	if (!tusb422_pd) {
	...
	tusb422_pd->tusb422_pinctrl = devm_pinctrl_get(tusb422_pd->dev);
	if (!tusb422_pd->tusb422_pinctrl) {
		dev_err(tusb422_pd->dev, "Target does not use pinctrl\n");
		return -EINVAL;
	}

	tusb422_pd->pinctrl_state_active = pinctrl_lookup_state(tusb422_pd->tusb422_pinctrl, PINCTRL_STATE_ACTIVE);
	if (!tusb422_pd->pinctrl_state_active) {
		dev_err(tusb422_pd->dev, "Can not lookup %s pinstate\n", PINCTRL_STATE_ACTIVE);
		return -EINVAL;
	}

	tusb422_pd->pinctrl_state_suspend = pinctrl_lookup_state(tusb422_pd->tusb422_pinctrl, PINCTRL_STATE_SUSPEND);
	if (!tusb422_pd->pinctrl_state_suspend) {
		dev_err(tusb422_pd->dev, "Can not lookup %s pinstate\n", PINCTRL_STATE_SUSPEND);
		return -EINVAL;
	}

	pinctrl_select_state(tusb422_pd->tusb422_pinctrl, tusb422_pd->pinctrl_state_active);
	return 0;
}
...
static int tusb422_of_get_gpios(struct tusb422_pwr_delivery *tusb422_pd)
{
	...
	tusb422_pd->usb_id = of_get_named_gpio(tusb422_pd->dev->of_node, "ti,usb-id", 0);
	if (gpio_is_valid(tusb422_pd->usb_id)) {
		rc = gpio_request(tusb422_pd->usb_id, "usb_id_gpio");
		if (rc)
		pr_err("tusb422 request gpio=%d fail!\n",tusb422_pd->usb_id);
	}
	else{
		pr_err("tusb422 get gpio name=%d fail!\n",tusb422_pd->usb_id);
		//tusb422_pd->usb_id = -EINVAL;
	}

	pr_info("tusb422 tusb422_pd->usb_id=%d\n",tusb422_pd->usb_id);
...
static int tusb422_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ret = tusb422_pinctrl_init(tusb422_pd);
	if (ret)
		goto err_of;
...
static int tusb422_pm_suspend(struct device *dev)
{
	int err = 0;
...
static int tusb422_pm_resume(struct device *dev)
{
	int err = 0;
...
static struct dev_pm_ops tusb422_pm_ops = {
	.suspend = tusb422_pm_suspend,
	.resume  = tusb422_pm_resume,
};
...
static void tusb422_shutdown(struct i2c_client *client)//add by longcheer_liml_2017_04_12
{
    printk("~~liml func=%s\n",__func__);
