...
static void gtp_pen_init(struct goodix_ts_data *ts)
{
    ...
    ts->pen_dev->phys = goodix_input_phys;
    ts->pen_dev->id.vendor = 0xDEAA;
    ts->pen_dev->id.product = 0xBEAF;
    ts->pen_dev->id.version = 10427;
	...
	if (g_charger_present) {
		if (!chr_pluggedin || dir_update) {
			...
			for (count = 0; count < 5; count++)
			{
				...
				mdelay(50);
				ret=gtp_i2c_read(ts->client, chr_state, 3);
				if((chr_state[2]&0x01))
					count = 5;
                GTP_INFO("chr_state[2] = %d\n",chr_state[2]);
...
static ssize_t gt91xx_config_read_proc(struct file *file, char __user *page, size_t size, loff_t *ppos)
{
	...
	*ppos += len;
    ...
    pr_info("%s::read total size is %zd\n", __func__, total_size);
...
/*			ret = gpio_request(power_ldo_gpio, "gt9xx_ldo_gpio");
			if (ret) {
				printk("gt9xx power ldo gpio request failed\n");
				return ret;
			}
*/
...
//    GTP_INFO("GTP Driver Built@%s, %s", __TIME__, __DATE__);
	...
	{
		init_tp_fm_info(0, fw_version, "GIS");
	}
	else
	{
