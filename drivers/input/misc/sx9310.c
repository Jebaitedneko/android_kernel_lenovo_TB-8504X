...
static ssize_t capsensor_config_read_proc(struct file *file, char __user *buf, size_t size, loff_t *ppos)
{/*
    char *ptr = page;
    ...
    */
	...
	page = kzalloc(8, GFP_KERNEL);

	count = sprintf(page,"%u\n", sx9310_enable);
	ret = simple_read_from_buffer( buf,size, ppos, page, count);
	printk("%s,page = %ld,ret = %u,count = %d\n",__func__, sizeof(page), ret,count);
	kfree(page);
	return ret;
}
