...
static unsigned int en_gpio;
static ssize_t vibr_on_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	return 0;
}


static ssize_t vibr_on_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int num;
	int ret;
	...
	if(ret)
	{
		printk("[vbr] str change to int failed, %d\n", ret);
		return ret;
	}
	...
	else if(num == 0)
	{
		...
		printk(KERN_ERR "[vbr]type wrong num!\n");
...
static DEVICE_ATTR(vibr_on, 0644, vibr_on_show, vibr_on_store);
...
static int vibrator_gpio_probe(struct platform_device *pdev)
{
	...
	if (ret < 0)
        ...
        ret = device_create_file(pdata->timed_dev.dev, &dev_attr_vibr_on);
	if(ret < 0)
	printk(KERN_ERR "[LYH] failed to create vibr_on!\n");
...
static int vibrator_gpio_remove(struct platform_device *pdev)
{
	...
	gpio_free(pdata->gpio);
        ...
        device_remove_file(pdata->timed_dev.dev, &dev_attr_vibr_on);
